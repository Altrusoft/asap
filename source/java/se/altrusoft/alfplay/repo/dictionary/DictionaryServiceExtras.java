...
public class DictionaryServiceExtras {
	// Inspired by org.alfresco.repo.web.scripts.dictionary.DictionaryWebServiceBase
	// in alfresco-remote-api-src

	private static final String NAME_DELIMITER = ":";
	
    private static Log logger = LogFactory.getLog(DictionaryServiceExtras.class);
	...
	public void init() {
		logger.info("Alfresco Play DictionaryServiceExtras is initializing");
	
		simpleQNames= new HashMap<String,SimpleQNameImpl>();
		qNamesToSimpleQNames= new HashMap<QName,SimpleQNameImpl>();
		
		logger.info("Alfresco Play DictionaryServiceExtras has been initialized");
	}
	...
	public Collection<PropertyDefinition> getPropertyDefinitions(ClassDefinition classDefinition) {
		return classDefinition.getProperties().values();
	}
	
	public Collection<ChildAssociationDefinition> getChildAssociationDefinitions(ClassDefinition classDefinition) {
		return classDefinition.getChildAssociations().values();
	}
	
	public Collection<AssociationDefinition> getAssociationDefinitions(ClassDefinition classDefinition) {
	...
	public SimpleQName getSimpleQName(QName qname) {
		...
		if (result == null) {
			...
			try {
				Collection<String> prefixes = namespaceService.getPrefixes(prefixUrl);
	...
	public SimpleQName getSimpleQName (String simpleQNameString)
	{
		...
		if (result == null) {
			...
			{
				logger.warn("Request for Simple QName with invalid argument "+ simpleQNameString, e);
			}	
		}		
		return result;
	}
	...
	public SimpleQName getSimpleQName (String prefix, String shortName)
	{
		...
		if (result == null) {
			...
			{
				logger.warn("Request for Simple QName with invalid arguments "+ prefix +", "+shortName, e);
			}	
		}		
		return result;
	...
	private HashMap<String, SimpleQNameImpl> simpleQNames;
	private HashMap<QName, SimpleQNameImpl> qNamesToSimpleQNames;
    ...
    private class SimpleQNameImpl implements SimpleQName {
    	...
    	private SimpleQNameImpl (String simpleQNameString) {   		
    		...
    		if (index > 0) {
    			...
    			NAME_DELIMITER+"found) :" + simpleQNameString);
    		}
    	}
